...
class CNN(Operation):
    ...
    def __repr__(self):
        ...
        return "%s|F:%0.2f|K:%d|A:%s|D:%0.2f" % (self.type(), self.filter_mul, self.k_size, self.activation, dropout)
    ...
    def decode(self, input_tensor):
        ...
        if activation in ['relu', 'sigmoid', 'tanh', 'elu']:
            x = Conv2D(filters, k_size, activation=activation, padding='same', kernel_initializer='he_normal',
                   kernel_regularizer=init)(input_tensor)
        elif activation == 'prelu':
            x = Conv2D(filters, k_size, padding='same', kernel_initializer='he_normal',
                   kernel_regularizer=init)(input_tensor)
            x = PReLU()(x)
        else:
            x = Conv2D(filters, k_size, padding='same', kernel_initializer='he_normal',
